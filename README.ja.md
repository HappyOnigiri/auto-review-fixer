...
- `.refix-batch.yaml` の `repositories` フィールドは不要です。
...
- `.refix-batch.yaml` 設定ファイル（バッチモード用）
...
2. `.refix-batch.sample.yaml` をもとに `.refix-batch.yaml` を編集します。
...
`refix` はリポジトリルートの `.refix-batch.yaml` を読み込みます。別パスを使う場合は `--config` で指定できます。
...
**対象リポジトリ**（Refix が管理するリポジトリ）のルートに `.refix.yaml` を配置することで、Refix がそのリポジトリをクローンまたは更新した後に実行するセットアップコマンドを定義できます。
...
コメント付きのテンプレートはこのリポジトリの `.refix.sample.yaml` を参照してください。
...
- `REFIX_CONFIG_YAML`
  ...
  - ローカルの `.refix-batch.yaml` に書く内容をそのまま保存してください。
