...
- **品質保証 (CI)**: コードを変更した後、コミットする前に必ず `make ci` を実行すること。失敗したタスクがあればエラーを修正し、全タスクが `SUCCESS` になるまで `make ci` を繰り返すこと。`make ci` が通った後にコミットに進むこと。`make ci` には ruff format・ruff check --fix・fix_newlines・check_non_ascii・check_dict_any・mypy・pyright・pytest が含まれる。なお ruff format / ruff check --fix / fix_newlines はファイルを自動修正するため、修正後のファイルもコミットに含めること。
